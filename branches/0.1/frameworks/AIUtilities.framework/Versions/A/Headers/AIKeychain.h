...
/*!	@brief Retrieves a password from the default keychain for a specified service/account combination.
 *
 *	Retreives a password from the default keychain for a specified service/account combination, requesting authorization from the user to access the keychain if necessary and allowed. Uses the Internet Password mechanisms.
 *	@param service An \c NSString identifying the service for this password.
 *	@param account An \c NSString identifying the account for this password.
 *	@return The requested password as an \c NSString, or \c nil if no password was found or the user denied keychain access.
*/
+ (NSString *)getPasswordFromKeychainForService:(NSString *)service account:(NSString *)account;

/*!	@brief Stores a password in the default for a specified service/account combination.
 *
 *	Stores a password in the default keychain for a specified service/account combination, requesting authorization from the user to access the keychain if necessary and allowed. Uses the Internet Password mechanisms.
 *
 *	@param service An \c NSString identifying the service for this password.
 *	@param account An \c NSString identifying the account for this password.
 *	@param password A \c NSString of the password to store.
 *	@return \c YES if storage was successful; \c NO if not.
 */
+ (BOOL)putPasswordInKeychainForService:(NSString *)service account:(NSString *)account password:(NSString *)password;

/*!	@brief Removes a password from the keychain for a specified service/account combination.
 *
 *	Removes a password from the default keychain for a specified service/account combination, requesting authorization from the user to access the keychain if necessary and allowed.
 *
 *	@param service An \c NSString identifying the service for this password.
 *	@param account An \c NSString identifying the account for this password.
 *	@return \c YES if removal was successful; \c NO if the service/account combination was not found or removal was unsuccessful.
 */
 ...
 *
 *	Retrieves a keychain dictionary for a given key. See \c AISystemNetworkDefaults for an example useage.
 *
 *	@param key The key by which to retrieve the dictionary.
 *	@return An \c NSDictionary of the values from the keychain.
 */
