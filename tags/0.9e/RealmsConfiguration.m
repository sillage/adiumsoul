 ...
 * Modified the -load method, to allow passing the file we want to load/write
 * by using -initWithConfigurationPathString:
 */
...
- (id)initWithConfigurationPathString:(NSString *)configurationFilePath
{
    if ((self = [super init])) {
        krb5_error_code err = 0;
        
        useDNS = YES;
        configurationPathString = [configurationFilePath retain];
        profile = NULL;
        realmsArray = NULL;
...
- (id)init
{
    return [self initWithConfigurationPathString:nil];
}
...
- (krb5_error_code) load
{
    ...
    if (!profile) {
        ...
        {
            NSString*   path = [configurationPathString stringByExpandingTildeInPath];
		...
		if (!paths || !*paths) { newConfigurationPath = @"/dev/null"; }
...
//			if (!*paths) { newConfigurationPath = @"/Library/Preferences/edu.mit.Kerberos"; }
            if (configurationPathString && (!paths || !*paths))
            {
                newConfigurationPath = [configurationPathString retain];
            }
//            if (configurationPathString) { [configurationPathString release]; }
...
- (krb5_error_code) flush
{
    ...
    if (!err && modified) {
		...
		if (err) {
            ...
            if (!err) {
                toolString = [NSString stringWithFormat: @"%@/%@", resourcePath, @"save_new_profile"];
            ...
            if (!err) {
                ...
                if (pid == -1) { 
                    ...
                    {
                        err = errno;
                    } // okay if Security.framework already waited 
                } else {
